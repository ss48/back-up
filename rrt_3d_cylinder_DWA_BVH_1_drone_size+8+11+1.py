...
from scipy.interpolate import splprep, splev  # For splining
...
def smooth_path_with_spline(path, smoothing_factor=0):
    """
    Applies spline smoothing to a path for a more natural, collision-free trajectory.
    """
    path_np = np.array(path)
    tck, u = splprep([path_np[:, 0], path_np[:, 1], path_np[:, 2]], s=smoothing_factor)
    u_fine = np.linspace(0, 1, len(path) * 5)  # Increase for finer path
    smoothed_path = np.array(splev(u_fine, tck)).T
    return smoothed_path

# Apply spline smoothing to all cube paths
cube1_path = smooth_path_with_spline(alns_optimized_path)
cube2_path = smooth_path_with_spline(optimized_path)  # Or path for cube2
cube3_path = smooth_path_with_spline(path)  # Or path for cube3

# Assign smoothed paths to cube positions
cube_positions = [cube1_path, cube2_path, cube3_path]
